...
import (
	...
	"github.com/matthewmueller/bud/router/internal/lex"
	"github.com/matthewmueller/diff"
)

func print(input string) string {
	return lex.Print(input)
}

func equal(t testing.TB, input, expected string) {
	t.Helper()
	actual := print(input)
	if actual == expected {
		return
...
func TestSample(t *testing.T) {
	...
	equal(t, `/{na me}`, `/ { slot:"na" error:"invalid character ' ' in slot" slot:"me" }`)
...
func TestAll(t *testing.T) {
	...
	equal(t, "/{a-b}", `/ { slot:"a" error:"invalid character '-b' in slot" }`)
	equal(t, "{a}", `error:"path must start with a slash /"`)
	equal(t, "/{a}/", `/ { slot:"a" } /`)
	...
	equal(t, "/{sLot}", `/ { slot:"s" error:"invalid character 'Lot' in slot" }`)
