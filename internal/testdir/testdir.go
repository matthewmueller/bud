...
import (
	...
	"github.com/matthewmueller/bud/internal/testlog"
...
type Dir struct {
	...
	to         virtual.FS
	cache      map[string][]byte
	writeFiles func(log logger.Log, from fs.FS, to virtual.FS) error
}
...
const goMod = `
	...
	require (
		github.com/matthewmueller/bud v0.0.0
	)
`
...
func (d *Dir) loadFS() (virtual.Tree, error) {
	...
	modFile.AddReplace("github.com/matthewmueller/bud", "", budDir, "")
	...
	if len(d.NodeModules) > 0 {
		nodePackage := &npmPackage{
			Name:         "testdir",
			Version:      "0.0.0",
			Dependencies: map[string]string{},
		}
		for name, version := range d.NodeModules {
			// TODO: re-enable JS runtime
...
func copyFiles(log logger.Log, from fs.FS, to virtual.FS) error {
	log.Debug("testdir: copying")
	return virtual.Copy(log, from, to)
}

// Sync files into a directory.
func syncFiles(log logger.Log, from fs.FS, to virtual.FS) error {
