...
func Sync(log logger.Log, from fs.FS, to FS, subpaths ...string) error {
...
func diff(log logger.Log, from fs.FS, to FS, dir string) (ops []syncOp, err error) {
...
func createOps(log logger.Log, from fs.FS, dir string, des []fs.DirEntry) (ops []syncOp, err error) {
	for _, de := range des {
		if de.Name() == "." {
			continue
		}
...
func deleteOps(log logger.Log, dir string, des []fs.DirEntry) (ops []syncOp, err error) {
	for _, de := range des {
...
func updateOps(log logger.Log, from fs.FS, to FS, dir string, des []fs.DirEntry) (ops []syncOp, err error) {
	for _, de := range des {
		if de.Name() == "." {
			continue
		}
...
func apply(log logger.Log, to FS, ops []syncOp) error {
	for _, op := range ops {
		switch op.Type {
		case createType:
